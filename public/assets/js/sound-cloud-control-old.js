...
if (url.toString().indexOf("soundcloud.com") !== -1) {
    ...
    setTimeout(function () {
        ...
        var current_start_time = document.getElementById('current_start_time').value;
        var Unix_current_start_time = new Date(current_start_time);
        var current_video_duration = parseInt(document.getElementById('current_video_duration').value * 1000);


        var now = new MyDate();
        var difference = Math.abs(now.nowUtcUnix() - Unix_current_start_time);
        ...
        if (SC.Widget.Events.READY == 'ready') {
            ...
            document.querySelector('#mute-div').addEventListener('click', function () {
